...
window.addEventListener('keyup', function () {
    if (event.keyCode === 13) { // if the enter key is pressed
        ...
        if (inputElements.includes(document.activeElement) ) { // if the user is working with the form
            addExpenseButton.click() // click the add expense button
            expenseNameInput.focus() // make expense name input active
        }
    }
    // If there's no active element in the page, nothing will happen when pressing enter
})